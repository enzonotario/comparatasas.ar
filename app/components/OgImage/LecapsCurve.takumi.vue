...
function fitPolyCurve(points: [number, number][], degree: number, n: number, minX: number, maxX: number) {
...
const chartConfig = computed(() => {
  ...
  const scaleY = (y: number) => height - padding - (y - minTir) / rangeTir * (height - 2 * padding)
  ...
  >
    <div
      :style="{
        position: 'absolute',
        inset: 0,
        background: 'linear-gradient(90deg, rgba(248, 250, 252, 0.98) 0%, rgba(248, 250, 252, 0.9) 50%, rgba(241, 245, 249, 0.7) 100%)',
      }"
    />

    <div :style="{ display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 1, flex: 1 }">
      <div :style="{ display: 'flex', alignItems: 'center', marginBottom: '20px' }">
        <span :style="{ fontSize: '20px', color: '#64748b', fontWeight: '600' }">ComparaTasas.ar</span>
      </div>
      ...
      <p v-if="updatedAt" :style="{ fontSize: '18px', color: '#64748b', margin: '0 0 30px 0' }">
      ...
      <div :style="{ display: 'flex', gap: '40px', flex: 1 }">
        ...
        <div :style="{ display: 'flex', flexDirection: 'column', gap: '15px', width: '350px' }">
          <h2 :style="{ fontSize: '20px', fontWeight: '700', color: '#1e293b', marginBottom: '5px' }">Mayores rendimientos (TIR)</h2>
          <div
          ...
          >
            <div :style="{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }">
              <span :style="{ fontSize: '18px', fontWeight: '700', color: '#0f172a' }">{{ item.symbol }}</span>
              <span :style="{ 
                fontSize: '12px', 
                fontWeight: '800', 
                padding: '2px 8px', 
                borderRadius: '6px',
                backgroundColor: item.type === 'BONCAP' ? '#dbeafe' : '#dcfce7',
                color: item.type === 'BONCAP' ? '#1e40af' : '#166534'
              }">{{ item.type }}</span>
            </div>
            <span :style="{ fontSize: '24px', fontWeight: '800', color: '#059669', marginTop: '5px' }">
              {{ formatPercent(item.tir || 0) }}
            </span>
          </div>
        </div>
        ...
        <div :style="{ display: 'flex', flexDirection: 'column', flex: 1 }">
           <h2 :style="{ fontSize: '20px', fontWeight: '700', color: '#1e293b', marginBottom: '15px' }">Curva de Rendimientos</h2>
           <div :style="{ 
             position: 'relative', 
             background: 'white', 
             borderRadius: '16px', 
             ...
             <svg v-if="chartConfig" width="100%" height="100%" viewBox="0 0 800 300" style="overflow: visible">
               ...
               <path :d="chartConfig.curvePath" fill="none" stroke="rgba(0, 0, 0, 0.2)" stroke-width="3" stroke-dasharray="8 4" />
               ...
               <g v-for="(p, i) in chartConfig.points" :key="i">
                 ...
                 <!-- Solo algunos labels para no saturar -->
                 <text 
                   v-if="i === 0 || i === chartConfig.points.length - 1 || i === Math.floor(chartConfig.points.length / 2)"
                   :x="p.x" 
                   :y="p.y - 15" 
                   text-anchor="middle"
                   style="font-size: 13px; font-weight: 700; fill: #475569"
                 >
                   {{ p.symbol }}
                 </text>
               </g>
             ...
             <div :style="{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', padding: '0 20px' }">
               <span :style="{ fontSize: '12px', color: '#64748b', fontWeight: '600' }">← Menor plazo</span>
               <span :style="{ fontSize: '12px', color: '#64748b', fontWeight: '600' }">Mayor plazo →</span>
             </div>
           </div>
