...
const { criptopesos, loading, error, fetch } = useCriptopesos()

const { calculateResults, isSimulating } = useInvestmentSimulator()
const criptopesosWithSimulation = calculateResults(criptopesos)

const { data: ogItems } = await useAsyncData('og-criptopesos', async () => {
  await fetch()
  return criptopesos.value.slice(0, 3).map((item) => ({
    name: `${item.typeLabel} / ${item.fondo}`,
    rate: formatTna(item.tna * 100),
  }))
})

defineOgImage('ComparaTasas.takumi', {
  title: 'Top Criptopesos',
  items: ogItems.value ?? [],
